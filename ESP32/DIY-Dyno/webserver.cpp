...
void Webserver::begin()
{
  ...
  server->on("/api/graph/download/liftdata.json", HTTP_GET, [](AsyncWebServerRequest *request){              
      ...
      AsyncWebServerResponse *response = request->beginResponse(200, asyncsrv::T_application_json, _webserver.getLiftDataJSON());
...
void Webserver::saveLiftDataForm(AsyncWebServerRequest *request){
  ...
  if (switchval >= 1 && switchval <= 12) {
    int idx = switchval - 1;
    valveData.datasets[idx].lift = (sensorVal.Swirl > 0.0) ? sensorVal.Swirl : (switchval * settings.valveLiftInterval);
...
String Webserver::processDatagraphPageTemplate(const String &var) {
  ...
  // Datagraph Stuff (Torque and HP Auto-scaling)
  int maxval = 100;
  double scaleFactor = 5.0;

  double max_metric = 10.0;
  for (int i = 0; i < 12; i++) {
    if (valveData.datasets[i].flow > max_metric) {
      max_metric = valveData.datasets[i].flow; // Hp
    }
    if (valveData.datasets[i].pdiff > max_metric) {
      max_metric = valveData.datasets[i].pdiff; // Torque
    }
  }

  // Determine auto scale dynamically
  if (max_metric <= 100.0) {
    maxval = 100;
    scaleFactor = 5.0; // 500px / 100 = 5.0
  } else if (max_metric <= 250.0) {
    maxval = 250;
    scaleFactor = 2.0; // 500px / 250 = 2.0
  } else if (max_metric <= 500.0) {
    maxval = 500;
    scaleFactor = 1.0; // 500px / 500 = 1.0
  } else {
    maxval = 1000;
    scaleFactor = 0.5; // 500px / 1000 = 0.5
  }

  // scale the data graph axis labels
  if (var == "flow1") return String(maxval / 10 * 1);
  ...
  // Return Torque Data coordinates (Red curve)
  for (int i = 1; i <= 12; i++) {
    if (var == "TORQUE_DATA" + String(i)) {
      return String(500 - (int)(valveData.datasets[i - 1].pdiff * scaleFactor));
    }
  }

  // Return Horsepower Data coordinates (Blue curve)
  for (int i = 1; i <= 12; i++) {
    if (var == "HP_DATA" + String(i)) {
      return String(500 - (int)(valveData.datasets[i - 1].flow * scaleFactor));
    }
  }

  // scale the datapoint values to fit the flow axis scale (Backwards compatibility)
  ...
  // Lift Profile (repurposed as RPM points on Dyno)
  for (int i = 1; i <= 12; i++) {
    String placeholder = "lift" + String(i);
    if (var == placeholder) {
      if (valveData.datasets[i - 1].lift > 0.0) {
        return String((int)valveData.datasets[i - 1].lift);
      } else {
        return String((int)(i * settings.valveLiftInterval)); // default interval in RPM
      }
    }
  }
